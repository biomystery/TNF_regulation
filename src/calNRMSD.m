function  [rmsd,nrmsd] = calNRMSD(simData,expData,sigmaRatioTh)
...
%      cols same as simData
    ...
    rmsd = sqrt(sum(sum((simData - expData_mean).^2))/numel(simData));
    nrmsd = sqrt(sum(sum(((simData - expData_mean)./expData_sigma).^2))/numel(simData));
end