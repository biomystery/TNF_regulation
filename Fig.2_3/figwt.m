...
for i =1:N % different pr vs same pr. 
    ...
    residues = calScores(input_pars,nfkb_exp,expData1,0);
    residues = [residues;calScore(input_pars,nascent_exp,expData2,0)];
...
if plot_flag
    ...
    xlabel('k_{pr}fold_{tko}');ylabel('V_{tr}');
    h = colorbar;
    ylabel(h,'RMSD')
end 
...
subplot 132
calScores([pr_rate(i(1)),km_rate(j(1))],nfkb_exp,expData1,1);
text(50,1.8,strcat('RMSD=',num2str(score(i,j))))
text(50,1.6,strcat('V_{tr}=',num2str(pr_rate(i)),',k_{pr}fold_{tko}=',num2str(km_rate(j))))

xlim([0 121]);ylim([0 2])
xlabel('Time (min)');ylabel('nascent (a.u.)')

subplot 133
calScore([pr_rate(i) km_rate(j)],nascent_exp,expData2,1);
xlim([0 120]);ylim([0 1.4])
xlabel('Time (min)');ylabel('mRNA (a.u.)')

text(50,1.3,strcat('RMSD=',num2str(minimal_score)))
text(50,1.1,strcat('V_{tr}=',num2str(pr_rate(i)),',k_{pr}fold_{tko}=',num2str(km_rate(j))))
...
print -dpdf -painters fit_wttko.pdf
saveas(gca,'fit_wttko.fig')
close all; !open fit_wttko.pdf
save ./simData/fit_wttko.mat 