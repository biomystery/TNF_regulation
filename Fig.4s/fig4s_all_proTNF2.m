...
sec_rate =linspace(.5,5,10); % indeed is the fold 
score = zeros(N,M);
...
for j = 1:M
    ...
    residues = calScore2(input_pars,mRNA_exp,expData,0);
...
if plot_flag
    ...
    ylabel('kdeg_{p}(min^{-1})');xlabel('k_{tl}fold_{tko}');
    h = colorbar;
    ylabel(h,'RMSD')
end 
...
subplot 122; calScore2([degp_rate(i(1)),sec_rate(j)],mRNA_exp,expData,1);
...
text(20,1,strcat('k_{tl}fold_{tko}=',num2str(sec_rate(j(1)))))
...
print -dpdf -painters fig4s_all_proTNF2.pdf
saveas(gca,'fig4s_all_proTNF2.fig')
close all; !open fig4s_all_proTNF2.pdf

save ./simData/fig4s_all_proTNF2.mat 