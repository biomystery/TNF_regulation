...
for i =1:N % different pr vs same pr. 
    ...
    residues = [residues;calScoreb(input_pars,mRNA_exp,expData2,0)];
...
subplot 132; calScore([degp_rate(i(1)),sec_rate(j(1))],mRNA_exp,expData,1);
xlim([0 121]);ylim([0 1.4])
xlabel('Time (min)');ylabel('proTNF (a.u.)')

subplot 133;calScoreb([degp_rate(i),sec_rate(j)],mRNA_exp,expData2,1);
xlim([0 121]);ylim([0 1.4])
